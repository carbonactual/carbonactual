...
test('authority/policy gate has explicit independent evaluators', () => {
  ...
  assert.match(gate, /AUTHORITY_CONTEXT_EXPIRED/);
...
test('execution gateway cannot execute blocked or malformed work', () => {
  assert.match(execution, /gateDecision !== 'ALLOW'/);
  assert.match(execution, /invalidEnvelope/);
  assert.match(execution, /HUMAN_AUTHORIZATION_REQUIRED/);
  assert.match(execution, /actionExecutor\.execute/);
});

test('execution requires evidence before canonical event persistence and reports persistence failures', () => {
  assert.match(execution, /EXECUTION_EVIDENCE_REQUIRED/);
  assert.match(execution, /CANONICAL_EVENT_PERSISTENCE_FAILED/);
