...
BEGIN
  ...
  CASE NEW.event_type
    WHEN 'agent_registered' THEN
      ...
      END IF;
      v_required_level := 3;
      UPDATE public.hapi_agents
      SET current_state = 'REGISTERED', updated_at = now()
      WHERE agent_id = NEW.actor_entity_id;
    ...
    WHEN 'agent_capabilities_bound' THEN
      ...
      v_required_level := 3;
      UPDATE public.hapi_agents
      SET current_state = 'PROVISIONED', updated_at = now()
      WHERE agent_id = NEW.actor_entity_id;
    ...
    WHEN 'agent_task_started' THEN
      ...
      v_required_level := 1;
      UPDATE public.hapi_agents
      SET current_state = 'EXECUTING', updated_at = now()
      WHERE agent_id = NEW.actor_entity_id;
    ...
    WHEN 'agent_terminated' THEN
      v_required_level := 4;
      UPDATE public.hapi_agents
      SET current_state = 'TERMINATED', updated_at = now()
      WHERE agent_id = NEW.actor_entity_id;
